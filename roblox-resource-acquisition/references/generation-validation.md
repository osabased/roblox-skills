...
Whenever a child is added, refreshed, repaired, or adopted, run `scripts/validate_skill_catalog.py <skill-directory-or-root> [...]` against the target generated-child set. Duplicate identities/descriptions fail. Reported activation-overlap clusters require the catalog behavioral tests in [testing-protocol.md](testing-protocol.md); static PASS never proves host routing.
